...
class Admission(models.Model):
    ...
    o86y = models.FileField(
        verbose_name="Справка о состоянии здоровья формы 086y",
        upload_to='o86y/')
    shg = models.FileField(
        verbose_name="Справка о группе здоровья",
        upload_to='shg/')
    class_field = models.CharField(
        verbose_name="Класс для поступления",
        max_length=255)
    specialization = models.CharField(
        verbose_name="Специализация ребёнка (с 8 класса)",
        max_length=255)
    essay = models.FileField(
        verbose_name="Эссе ребёнка о своих увлечениях(от 240 слов)",
        upload_to='essay/')
    testing = models.FileField(
        verbose_name="Результаты тестирования",
        upload_to='testing/')
    exam = models.FileField(
        verbose_name="Результаты вступительного экзамена",
        upload_to='exam/')
    ...
    status = models.BooleanField(
        verbose_name="Статус рассмотрения заявки",
        default=False)
